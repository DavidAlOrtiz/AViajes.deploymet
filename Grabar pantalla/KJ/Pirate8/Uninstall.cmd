...
1>nul 2>nul copy /y P\mui\%mui%\ActionCenter.dll.mui %SysPath%\%mui%\ActionCenter.dll.mui && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
echo.
echo. Copying %mui% GenuineCenter.dll.mui files ...
1>nul 2>nul copy /y P\mui\%mui%\GenuineCenter.dll.mui %SysPath%\%mui%\GenuineCenter.dll.mui && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
echo.
echo. Copying %mui% systemcpl.dll.mui files ...
1>nul 2>nul copy /y P\mui\%mui%\systemcpl.dll.mui %SysPath%\%mui%\systemcpl.dll.mui && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
echo.
echo. Copying %mui% SystemSettings.exe.mui files ...
1>nul 2>nul copy /y P\mui\%mui%\SystemSettings.exe.mui %Windir%\ImmersiveControlPanel\%mui%\SystemSettings.exe.mui && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
...
if exist P\x86\%FileName% copy /y P\x86\%FileName% %SysPath%\%FileName% && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
...
if exist P\x64\%FileName% copy /y P\x64\%FileName% %SysPath%\%FileName% && echo       status: OK
IF ERRORLEVEL 1 echo       status: Error
if exist %SysPath%\%FileName%.vvv move %SysPath%\%FileName%.vvv %Temp%\%Random%.tmp >nul
set /A i=i+1
...
:_end
echo.
echo. Uninstalling, Windows Activation Technologies ...
echo. Working, Please wait ...
...
call %SysPath%\regedt32.exe /s P\Activation.reg
copy /y ac.cmd "%userprofile%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\ac.cmd" >nul
takeown /f "%SysPath%\sppsvc.exe" >nul && icacls "%SysPath%\sppsvc.exe" /grant *S-1-5-19:F >nul
cls
set i=5
:Rebooting
cls
